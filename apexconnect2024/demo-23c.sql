...
-- ---------------------------------------------------------------------------------------
--  ORACLE23AI JSON-schema + APEX-extension
-- ---------------------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------------------
--  ORACLE23AI Describe JSON-schema
-- ---------------------------------------------------------------------------------------

set pagesize 1000
SELECT json_serialize(DBMS_JSON_SCHEMA.describe('OBJECT23AI', null, 'DATA') pretty);


-- ---------------------------------------------------------------------------------------
--  ORACLE23AI JSON-Relational-Duality
-- ---------------------------------------------------------------------------------------

drop view if exists json2ai;

CREATE OR REPLACE json relational duality view json23ai AS
SELECT JSON { '_id': o.objecT_id,
...
------------
SELECT json_serialize(DBMS_JSON_SCHEMA.describe('JSON23AI') PRETTY);

select rowid, ora_rowscn, DATA from JSON23AI where rownum<=1;

select rowid, ora_rowscn, t.* from object t;

update /*+qb_name(apex$100_14)*/(select rowid, DATA from JSON23AI with check option)
set "DATA"=:APEX$VAL2
...
insert into json23ai values('{"object_name": "new object", "object_type_name": "test-string-1"}');
